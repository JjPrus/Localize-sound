...
def get_audio(chunk, channels, rate, record_time, indeks, queue):
...
def obliczenia(x_val, y_val, line1, wx, wy, queue):
    while True:
        if not queue.empty():
            nazwa = str(queue.get())
            sample_rate, data = read(nazwa)
...
def dalej():
    ...
    # Uruchamianie multiprocessingu
    if __name__ == '__main__':

        queue = Queue()

        get_audio_process = Process(target=get_audio, args=(1024, 6, 44100, 1, toto, queue))
        obliczenia_process = Process(target=obliczenia,args=(x_val, y_val, line1, wx, wy, queue))
